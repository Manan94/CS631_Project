...
<body>
	...
	<?php
		...
		$sql = "SELECT PID, PName, PPrice, Description from Product where PType='A' and PQuantity > 0";
		$result = $conn->query($sql);

		if ($result->num_rows > 0) {
			while($row = $result->fetch_assoc()) {
				echo "<tr><td>".$row["PName"]."</td><td>$".$row["PPrice"]."</td><td>".$row["Description"]."</td><td><input type='submit' value='Add to Cart'/></td></tr>";
			}
			echo "</table>";
		} else {
